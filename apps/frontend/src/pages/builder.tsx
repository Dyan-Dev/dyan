...
export default function BuilderPage() {
  ...
  const handleEdit = async (index: number) => {
    ...
    try {
      const res = await fetch(`http://localhost:3000/dyan/endpoint?path=${encodeURIComponent(ep.path)}&method=${ep.method}`);
      const data = await res.json();
  ...
  const handleDelete = async (index: number) => {
    ...
    try {
      ...
      if (selectedEndpoint?.path === ep.path && selectedEndpoint.method === ep.method) {
      ...
      <div className="flex-1 overflow-auto p-6 space-y-6">
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-2">
            <Button onClick={handleTest} variant="outline" className="gap-1" disabled={!selectedEndpoint}>
